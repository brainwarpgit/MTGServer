...
void BoxVolume::read(IffStream *iff) {
	...
	if (version == '0000' && (bounds == nullptr || bounds->getChunkSize() != 24)) {
		throw Exception(String("Invalid legacy BOX chunk in ") + iff->getFileName());
	}
	...
	if (version == '0000') {
		// Legacy boxes store their enclosing sphere directly after the bounds.
